function RMSE = ImgSFreqComp_v4(noiseImPars)
...
[im1, noiseData] = noiseBaseImDescFile(0); % call with option 0 to get the global variables..
...
if noiseData.imgSize == "match"
%Make the size match the base image..
noiseData.imgSize   = [im1_size(1), im1_size(2)];      % Size of noise image (enter 2 values if x & y dims are different, ex: [256,512] )
end

% What Type Noise?
noizType = noiseData.NoiseType; % check the type of noise and save locally as "noizType"..

% replace parameters in struct with the input parameters to be optimized.. 
% .. based on the type of noise we're running..
switch  noizType
    % if NoiseType is "gabor" unpack the gaussian blobs stuff...
    case "gabor"
