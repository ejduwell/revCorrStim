...
mlab_dir = fileparts(which('RevCorrCIGen_v4.m'));
startDir=pwd;
cd(mlab_dir);
...
ciThr=70; % percent threshold used for thesholded/colorized CI and ideal CI overlays..

oriConFlp=1; % if 1, will flip all noise from one of the two conditions allon axis specified by before making CI.. (adam's request)
flpAxis=2; % axis along which flip will occur if oriConFlp is on/=1. (1=vertical, 2=horiz)
FlpdOriCon=2; % which orientation condition gets flipped? (1 or 2)
...
%ciThr=80; % percent threshold used for thesholded/colorized CI and ideal CI overlays..
