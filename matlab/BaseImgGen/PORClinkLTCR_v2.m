function [backwindow] = PORClinkLTCR_v2(mainsrc, window, backwindow,vertOcc,horzOcc,boxUL,boxUR,boxLL,boxLR,gray,gray1,gray2,trialprop,occ,CR,CR_penWidth,CR_penLum,CR_al,CR_obj,scrnWidHei,rscale_f1, rscale_f2, image_in, db_mode,normalize,norm_range,maindim,T,T_al,L,L_al,lumWt,cr_long,cr_short,cr_CornerRad,cmdLineOut)
...
%grey_base = one_base * grey; %EJD COMMENTED..
grey_base = one_base * gray1;
...
%Then put the masked image shapes into the "cookie trays" (grey backgroundcr_CornerRad
%masks)
...
nPnts=100; %Number of points in the corner arcs (controls smoothness of rounded corners)
pointsOut = roundedRectangle(CRbox(1), CRbox(2), CRbox(3), CRbox(4), cr_CornerRad,nPnts);
Screen('FramePoly', backwindow, CR_penLum, pointsOut,penWidth);
%Screen('FrameArc',backwindow,CR_penLum,CRbox,0,360, penWidth, penWidth)
%Screen('FrameOval', backwindow, CR_penLum, CRbox,penWidth);
end
