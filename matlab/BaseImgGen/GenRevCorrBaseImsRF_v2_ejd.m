function GenRevCorrBaseImsRF_v2_ejd
...
thisFile = "GenRevCorrBaseImsRF_v2_ejd.m"; % string should correspond to the name of this file's name..
...
%% Parameters
    ...
    cropsz = 128; %output edge dimension after cropping into square

    %% System Parameters
    %--------------------------------------------------------------
    outdir = "Lum_Tex_StdBaseImgs"; % Specify output directory (which will be created)
    ...
    image_in = 'spheres_pic_rs5_nearest.jpg';
    ...
    rscale_f1 = linspace(0.5,2,4); % texture scaling factor 
    %rscale_f2 = 0.5; % texture scaling factor 2
    rscale_f2 = linspace(0.5,2,4); % texture scaling factor 2
    ...
    %lumWt = linspace(0.5,0.5,1); % luminance weight (for texture/luminance combo.. note: texture weight will be 1-lumWt..) (num between 0-1)
    ...
    %avoidEqlz = {"lum1==lum2","rscale_f1==rscale_f2"}; % specify equality conditions you do now want
...
%imOutMat = RevCorrBaseImGen_v2(parsIn,window,change_screensize,scale_f,imOutMat);
imOutMat = RevCorrBaseImGen_v2(parsIn,window,change_screensize,scale_f,imOutMat,method);
