...
% %     respMat(2,zz)=tcon;
% 
%     % assign trial condition (l vs right) based on whether right or left
%     % similarity was greater.. if equal roll the dice..
%     ssmat{1,uu}=ssf = figure;
%     ssmat{2,uu}=ssimvalR;
%     if ssmat{2,zz}>ssmat{1,zz} % more similar to right angled version..
%         tcon = 0; % assign right
%     end
% 
%     if ssmat{2,zz}<ssmat{1,zz} % more similar to left angled version..
%         tcon = 1;% assign left
%     end
% 
%     if ssmat{2,zz}==ssmat{1,zz} % if by coincidence their ssims are exactly equal
%         tcon = randsample(respOps,1,true,[0.5, 0.5]); % roll dice..
%     end
    respMat(2,zz)=tcon;
...
axis tight
colorbar
title("SSIM CI-22 (mean)")

%% Nth percentile thresholding..

% Define the percentile
n = 95; % Change this to your desired value
