...
% Use a structure array as a temporary variable
tempResults(size(noiseImsComb, 2)) = struct('cL', [], 'cR', [], 'corrImL', [], 'corrImR', [], 'corValDiff', []);
...
parfor (uu = 1:size(noiseImsComb, 2), nWorkers)
    ...
    % Save results in the temporary structure
    tempResults(uu).cL = cL;
    tempResults(uu).cR = cR;
    tempResults(uu).corrImL = corrImL;
    tempResults(uu).corrImR = corrImR;
    tempResults(uu).corValDiff = corValDiff;
end

% Transfer results from the structure to the cell array
for uu = 1:size(noiseImsComb, 2)
    ssmat{1, uu} = tempResults(uu).cL;
    ssmat{2, uu} = tempResults(uu).cR;
    ssmat{3, uu} = tempResults(uu).corrImL;
    ssmat{4, uu} = tempResults(uu).corrImR;
    ssmat{5, uu} = tempResults(uu).corValDiff;
end
clear tempResults
