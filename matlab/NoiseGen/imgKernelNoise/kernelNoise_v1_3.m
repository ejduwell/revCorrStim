function [nzImz] = kernelNoise_v1_3(imgsPerKrnl,desiredSize,krnlNzBI,nReps,krnlImgWgts,smoothTiles,gSmthK1sz,smoothFinal,gSmthK2sz)

rng("shuffle"); % Make sure the random number generator is shuffled..
meanLumIn=mean(krnlNzBI,"all"); % compute mean luminance of input image..
maxLumIn=max(max(krnlNzBI));
minLumIn=min(min(krnlNzBI));
...
for yy = 1:kFrkn
    ...
    for xx = 1:kFrkn
        ...
        kSubImg=rotateKrnlWindow2(kCoords,adjImg);
        else
         kSubImg=uint8(adjImg(kCoords(2):kCoords(4),kCoords(1):kCoords(3)));   
        end
