function structOut = KrnlSmplNoise_v9_Demo_multi()
...
nWorkers=12; % number of cpu nodes used in the parallelized portions..
...
baseImgL="/home/eduwell/Matlab_Sandbox/imgKernelNoise/pyAlignFaceOutTest6/dir1/cropped_average_face.jpg"; 
...
baseImgR="/home/eduwell/Matlab_Sandbox/imgKernelNoise/pyAlignFaceOutTest6/dir2/cropped_average_face.jpg"; 
...
[nzImz] = kernelNoise_v1(imgsPerKrnl,desiredSize,krnlNzBI,nReps,krnlImgWgts,smoothTiles,gSmthK1sz,smoothFinal,gSmthK2sz);

%% Generate other noise type images for comparison
tic
WhtNzImz=uint8(zeros(desiredSize(1),desiredSize(2),nReps));
for kk = 1:nReps
whiteNoiseImg=uint8(randi(255,[size(krnlNzBI,1),size(adjImg,1)]));
...
% NEW
% Compute Ideal CIs
idealCI1=double(BI_L)-double(BI_R);
idealCI2=double(BI_R)-double(BI_L);
idealMax=max(max(idealCI1));
idealMin=min(min(idealCI1));

% % ORIG:
% % Load ideal CIs and ensure dimensions are those requested..
% % -------------------------------------------------------------------------
% % Read them in
% idealCI1=imread(idealCIPath1);
% idealCI2=imread(idealCIPath2);
% 
% % If images are not grayscale, convert
% ideal1Dimz=ndims(idealCI1); % get number of dimensions
% ideal2Dimz=ndims(idealCI2); % get number of dimensions
% if size(ideal1Dimz) > 2
%     idealCI1=rgb2gray(idealCI1);
% end
% if size(ideal2Dimz) > 2
%     idealCI2=rgb2gray(idealCI2);
% end
% 
% % ensure desired dimensions..
% idealCI1 = imgReformater(idealCI1,desiredSize);
% idealCI2 = imgReformater(idealCI2,desiredSize);
% % -------------------------------------------------------------------------
...
%% Save copy of "Ideal CIs" in Struct Output
structOut.idealCIs.idealCI1=idealCI1;
structOut.idealCIs.idealCI2=idealCI2;
