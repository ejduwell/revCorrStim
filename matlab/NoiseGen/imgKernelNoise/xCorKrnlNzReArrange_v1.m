function [kernelFrames, krnlWindwStax, tallyImg]= xCorKrnlNzReArrange_v1(baseIm, noiseIm, kernelDim_ImFrac, kernelSqrDims,MinMaxCor,nKernReArr)
...
tallyImg=zeros(size(baseIm,1),size(baseIm,2));
...
for aa=1:size(kStack,3)
    ...
    % Finally, record the fact this region matched in the tally image 
    % (ie add one to the pixels in this region in the tally img)
    tlyTmp=ones(size(matchedRegion,1),size(matchedRegion,2));
    tallyImg(CoordsTmp(1):CoordsTmp(2), CoordsTmp(3):CoordsTmp(4))=((tallyImg(CoordsTmp(1):CoordsTmp(2), CoordsTmp(3):CoordsTmp(4)))+tlyTmp);
