...
quests_ntrials_reg = 250; % specify number of trials in each quest test rep  on the quest block after the first block
...
%     qst_startParams.(qStringTmp).range = 1000;
    ...
    qst_startParams.(qStringTmp).tGuessSd = 0.2*(abs(maxmin{1,1}-maxmin{1,2}));  % set std. dev. to n% of the total possible parameter value range
    qst_startParams.(qStringTmp).grain = 0.01;
    qst_startParams.(qStringTmp).range = 2*(abs(maxmin{1,1}-maxmin{1,2})); % set quest range to 2 times total possible parameter range.
end
