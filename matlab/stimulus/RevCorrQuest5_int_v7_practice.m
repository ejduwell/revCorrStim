...
try
    ...
    key1 = KbName(trial_con(1));
    key2 = KbName(trial_con(2));
    ...
    fix_im_path = strcat(image_dir,"/","fixation.png");  %Image Path.. generalized
    ...
    % add formatted text..
    txtDistFrmTop1=37.5*scrn_1percY; % specify distance of text from top as percent of screen dimension..
    Screen('TextSize',window, intro_txtSize);
    speech1 = ['You will first complete a short block of 18 trials so you can practice the task. \n' ...
        ...
        'You will then complete one short 3-minute run followed by \n'...
        '8, 6.5-minute runs. You will not get feedback during these runs. \n'...
        'Between runs, you will be given a 30-second break, \n '...
        'and then the screen will prompt you to begin the next block by hitting the ENTER key. \n'...
        'However, feel free to take a longer rest, if you need one, before you continue. \n\n\n'...
    ...
    leavePractice=0; % initialize

    while leavePractice==0
        %Screen('FillRect', window, [grayblack grayblack grayblack 0]);  % blank out the screen
        Screen('FillRect', window, gray); % gray out window

        trialtm = zeros(5,numtrials);   % pre-allocate
        Screen('TextSize',window, intro_txtSize);
        speech1 = ' Press ENTER/RETURN to begin presentation of the practice trials..';
        ...
        for trials = 1:numtrials
            ...
            if maxmin_seg == 1
                ...
                if n_int == 2
                    seg1min=minparam;    %occCon_mat = zeros(numtrials,5);
                    seg1max=maxparam/2;
                    seg2min=maxparam/2;
                    seg2max=maxparam;
                end
            ...
            switch link
                case 'F'
                    ...
                    if swtch2 == 0
                        ...
                        respns = KbName(firstPress==max(firstPress));
                        nresp=sum(firstPress==max(firstPress));
                        if nresp>1
                            respns=respns{randi(nresp)};
                            resp_mat(trials,1) = respns; % in edge case of simultaneous multiple button press.. make a random selection
                        else
                            resp_mat(trials,1) = respns; % record button press
                        end
                    else
            ...
            if n_int > 1
                q2 = QuestUpdate(q2,log10(param1(trials)),response(trials,1));
            end
            if n_int > 2
                q3 = QuestUpdate(q3,log10(param1(trials)),response(trials,1));
            end
            if n_int > 3
                q4 = QuestUpdate(q4,log10(param1(trials)),response(trials,1));
            end
            if n_int > 4
                q5 = QuestUpdate(q5,log10(param1(trials)),response(trials,1));
            end
            ...
            if n_int>1
                ...
                switch link
                    case 'F'
                        ...
                        if swtch2 == 0
                            ...
                            if nresp>1
                                respns=respns{randi(nresp)};
                                resp_mat(trials,2) = respns; % in edge case of simultaneous multiple button press.. make a random selection
                            else
                                resp_mat(trials,2) = respns; % record button press
                            end
                        else
                ...
                % Add the new data
                q1 = QuestUpdate(q1,log10(param2(trials)),response(trials,2));
                if n_int > 1
                    q2 = QuestUpdate(q2,log10(param2(trials)),response(trials,2));
                end
                if n_int > 2
                    q3 = QuestUpdate(q3,log10(param2(trials)),response(trials,2));
                end
                if n_int > 3
                    q4 = QuestUpdate(q4,log10(param2(trials)),response(trials,2));
                end
                if n_int > 4
                    q5 = QuestUpdate(q5,log10(param2(trials)),response(trials,2));
                end
            ...
            if n_int>2
                ...
                switch link
                    case 'F'
                        ...
                        if swtch2 == 0
                            ...
                            if nresp>1
                                respns=respns{randi(nresp)};
                                resp_mat(trials,3) = respns; % in edge case of simultaneous multiple button press.. make a random selection
                            else
                                resp_mat(trials,3) = respns; % record button press
                            end
                        else
                ...
                switch fback_On
                    ...
                    case 0                        %answer = strcat(answer,' ','button pressed:', tdesc_file{trials,9});
                        if respns == tcon
                            response(trials,3) = 1;
                        else
                            response(trials,3) = 0;
                ...
                % Add the new data
                q1 = QuestUpdate(q1,log10(param3(trials)),response(trials,3));
                if n_int > 1
                    q2 = QuestUpdate(q2,log10(param3(trials)),response(trials,3));
                end
                if n_int > 2
                    q3 = QuestUpdate(q3,log10(param3(trials)),response(trials,3));
                end
                if n_int > 3
                    q4 = QuestUpdate(q4,log10(param3(trials)),response(trials,3));
                end
                if n_int > 4
                    q5 = QuestUpdate(q5,log10(param3(trials)),response(trials,3));
                end
            end
            %% Staircase 4
            if n_int>3
                ...
                switch link
                    case 'F'
                        ...
                        if swtch2 == 0
                            ...
                            if nresp>1
                                respns=respns{randi(nresp)};
                                resp_mat(trials,4) = respns; % in edge case of simultaneous multiple button press.. make a random selection
                            else
                                resp_mat(trials,4) = respns; % record button press
                            end
                        else
                ...
                % Add the new data
                q1 = QuestUpdate(q1,log10(param4(trials)),response(trials,4));
                if n_int > 1
                    q2 = QuestUpdate(q2,log10(param4(trials)),response(trials,4));
                end
                if n_int > 2
                    q3 = QuestUpdate(q3,log10(param4(trials)),response(trials,4));
                end
                if n_int > 3
                    q4 = QuestUpdate(q4,log10(param4(trials)),response(trials,4));
                end
                if n_int > 4
                    q5 = QuestUpdate(q5,log10(param4(trials)),response(trials,4));
                end
            end
            %% Staircase 5
            if n_int>4
                ...
                switch link
                    case 'F'
                        ...
                        if swtch2 == 0
                            ...
                            nresp=sum(firstPress==max(firstPress));
                            if nresp>1
                                resp_mat(trials,5) = respns{randi(nresp)}; % in edge case of simultaneous multiple button press.. make a random selection
                            else
                                resp_mat(trials,5) = respns; % record button press
                            end
                        else
                ...
                % Add the new data
                q1 = QuestUpdate(q1,log10(param5(trials)),response(trials,5));
                if n_int > 1
                    q2 = QuestUpdate(q2,log10(param5(trials)),response(trials,5));
                end
                if n_int > 2
                    q3 = QuestUpdate(q3,log10(param5(trials)),response(trials,5));
                end
                if n_int > 3
                    q4 = QuestUpdate(q4,log10(param5(trials)),response(trials,5));
                end
                if n_int > 4
        ...
        perCorrect=num2str((mean(cell2mat(tdf_out(2:end,12))))*100);
        accuracyStr = convertStringsToChars(strcat('Accuracy was: '," ",perCorrect,'% \n'));
        accuracyStr2 = convertStringsToChars(strcat('Accuracy was: '," ",perCorrect,'%'));

        speechend1 = strcat('Practice block has been completed.');
        starz = '*************************************************************';
        ...
        Screen('FillRect', window, gray); % gray out window
        % add formatted text..
        speech1 = ['You''ve completed the practice trials. \n'...
            accuracyStr ...
            'Please alert the experimenter. \n\n'...
            'When they arrive...\n\n'...
            'If you want to repeat the practice trials, press "r" \n\n' ...
            'If you think you are ready to continue, \n' ...
