function [trialtm,ts_out,qs_out,sd_out,nr_ideals, params_out, tdf_out] = FACEquest5_int_v2(room,kboard,numtrials,link,noise,window,image_dir,db_mode,db_mode_screen,nr_params, imtag, pthrds, n_int, qst_startParams,t_prob,maxmin,maxmin_seg,main_dir)
...
%     newres = res1(res4(rmax));
    ...
    switch link
        case 'F' %
            ...
            imFiles ={}; %initialize the image file list..
            ...
            for ii=1:size(imFiles,1)
                ...
                [~,name_only,~] = fileparts(fname); % separate the filename away from its path and extension...
                im_param = name_only((end-2):end); % extract the face warp parameter from the filename
                % !! WARNING !! : The line above is somewhat of a "magic number"
                % scenario.. only works with the current naming convention...
                % Currently assuming that the parameter is the last three characters
                % before the .bmp file extension.. need to update to generalize
                % or if you change filenaming convention..
                im_param = str2double(im_param); % convert from string to double
                img_array(ii,3) = {im_param};  % save image face warp param in
                % img_array cell array column3.
            end
            fwarp_inxs = cell2mat(img_array(2:end,3)); % save the parameters column for index referencing later (2:end bc we don't want to include the "Ape only" image at 1)
            ape_only_im = img_array{1,1}; % assign the "ape only" image to its own variable..
    ...
    %[backwindow_fix,backwindow] = FACEfix(backwindow_fix,backwindow,fixrect,grayblack);
    ...
    for trials = 1:numtrials
        ...
        switch link
            case 'F'
                ...
                %!!??
                % if its a human+ape trial (trial_con = h)...
                % Reset param1 to be the closest available value computed
                % above..
                if tcon==trial_con(2)
                    param1(trials)=closest_param;  % get next level of parameter to test
                    con_mat(trials,1) = "Mix_Quest1";
                    im_mat(trials,1) = img_array{idx,2}; % Save image name/path
                    MorphParMat(trials,1) = closest_param;
                    %!!??
                end

                % If its an ape only trial, reset param1 to be 1 (the ape only
                % parameter)
                if tcon==trial_con(1)
                    param1(trials)=1;
                    con_mat(trials,1) = "cApe";
                    im_mat(trials,1) = img_array{1,2}; % Save image name/path
                    MorphParMat(trials,1) = 1;
                end
                image_in = img_array{idx,1};

                % draw linking display to offscreen window
                [backwindow] = PORClinkF(mainsrc, window, backwindow,trialprop(trials,1), trial_con, image_in, ape_only_im,fixrect,grayblack);
            ...
            switch link
                case 'F'
                    ...
                    if tcon==trial_con(2)
                        ...
                        con_mat(trials,2) = "Mix_Quest2";
                        im_mat(trials,2) = img_array{idx,2}; % Save image name/path
                        MorphParMat(trials,2) = closest_param;
                        %!!??
                    end
                    ...
                    if tcon==trial_con(1)
                        param2(trials)=1;
                        con_mat(trials,2) = "cApe";
                        im_mat(trials,2) = img_array{1,2}; % Save image name/path
                        MorphParMat(trials,2) = 1;
                    end
                    image_in = img_array{idx,1};

                    % draw linking display to offscreen window
                    [backwindow] = PORClinkF(mainsrc, window, backwindow,trialprop(trials,2), trial_con, image_in, ape_only_im,fixrect,grayblack);
                    %Screen('DrawTexture', window, backwindow, mainsrc, maindest, 0, 0, 1);  % draw stimulus to the on-screen window, rotated & zoomed
                    Screen('DrawTexture', window, backwindow, mainsrc, maindest, 0, 0, 1);  % draw stimulus to the on-screen window, rotated & zoomed
                    %[backwindow_fix,backwindow] = FACEfix(backwindow_fix,backwindow,fixrect,grayblack,fix_im);
                    %Screen('DrawTexture', window, backwindow_fix, mainsrc, maindest, 0, 0);  % draw fixation screen to the on-screen window, rotated & zoomed
        ...
        if n_int>2
            ...
            switch link
                case 'F'
                    ...
                    if tcon==trial_con(2)
                        ...
                        con_mat(trials,3) = "Mix_Quest3";
                        im_mat(trials,3) = img_array{idx,2}; % Save image name/path
                        MorphParMat(trials,3) = closest_param;
                        %!!??
                    end
                    ...
                    if tcon==trial_con(1)
                        param3(trials)=1;
                        con_mat(trials,3) = "cApe";
                        im_mat(trials,3) = img_array{1,2}; % Save image name/path
                        MorphParMat(trials,3) = 1;
                    end
                    image_in = img_array{idx,1};

                    % draw linking display to offscreen window
                    [backwindow] = PORClinkF(mainsrc, window, backwindow,trialprop(trials,3), trial_con, image_in, ape_only_im,fixrect,grayblack);
                    %Screen('DrawTexture', window, backwindow, mainsrc, maindest, 0, 0, 1);  % draw stimulus to the on-screen window, rotated & zoomed
                    Screen('DrawTexture', window, backwindow, mainsrc, maindest, 0, 0, 1);  % draw stimulus to the on-screen window, rotated & zoomed
                    %[backwindow_fix,backwindow] = FACEfix(backwindow_fix,backwindow,fixrect,grayblack,fix_im);
                    %Screen('DrawTexture', window, backwindow_fix, mainsrc, maindest, 0, 0);  % draw fixation screen to the on-screen window, rotated & zoomed
            ...
            tim_mat(trials,3,3) = tim_mat2(trials,4,2)-strt; % save total trial time after ITI too..
            ...
            switch link
                case 'F'
                    ...
                    if tcon==trial_con(2)
                        ...
                        con_mat(trials,4) = "Mix_Quest4";
                        im_mat(trials,4) = img_array{idx,2}; % Save image name/path
                        MorphParMat(trials,4) = closest_param;
                        %!!??
                    end
                    ...
                    if tcon==trial_con(1)
                        param4(trials)=1;
                        con_mat(trials,4) = "cApe";
                        im_mat(trials,4) = img_array{1,2}; % Save image name/path
                        MorphParMat(trials,4) = 1;
                    end
                    image_in = img_array{idx,1};

                    % draw linking display to offscreen window
                    [backwindow] = PORClinkF(mainsrc, window, backwindow,trialprop(trials,4), trial_con, image_in, ape_only_im,fixrect,grayblack);
                    %Screen('DrawTexture', window, backwindow, mainsrc, maindest, 0, 0, 1);  % draw stimulus to the on-screen window, rotated & zoomed
                    Screen('DrawTexture', window, backwindow, mainsrc, maindest, 0, 0, 1);  % draw stimulus to the on-screen window, rotated & zoomed
                    %[backwindow_fix,backwindow] = FACEfix(backwindow_fix,backwindow,fixrect,grayblack,fix_im);
                    %Screen('DrawTexture', window, backwindow_fix, mainsrc, maindest, 0, 0);  % draw fixation screen to the on-screen window, rotated & zoomed
            ...
            switch link
                case 'F'
                    ...
                    if tcon==trial_con(2)
                        ...
                        con_mat(trials,5) = "Mix_Quest5";
                        im_mat(trials,5) = img_array{idx,2}; % Save image name/path
                        MorphParMat(trials,5) = closest_param;
                        %!!??
                    end
                    ...
                    if tcon==trial_con(1)
                        param5(trials)=1;
                        con_mat(trials,5) = "cApe";
                        im_mat(trials,5) = img_array{1,2}; % Save image name/path
                        MorphParMat(trials,5) = 1;
                    end
                    image_in = img_array{idx,1};

                    % draw linking display to offscreen window
                    [backwindow] = PORClinkF(mainsrc, window, backwindow,trialprop(trials,5), trial_con, image_in, ape_only_im,fixrect,grayblack);
                    %Screen('DrawTexture', window, backwindow, mainsrc, maindest, 0, 0, 1);  % draw stimulus to the on-screen window, rotated & zoomed
                    Screen('DrawTexture', window, backwindow, mainsrc, maindest, 0, 0, 1);  % draw stimulus to the on-screen window, rotated & zoomed
                    %[backwindow_fix,backwindow] = FACEfix(backwindow_fix,backwindow,fixrect,grayblack,fix_im);
                    %Screen('DrawTexture', window, backwindow_fix, mainsrc, maindest, 0, 0);  % draw fixation screen to the on-screen window, rotated & zoomed
    ...
    tdf_out = cell(numtrials*n_int,17);
    ...
    for ii = 1:numtrials
        for zz = 1:n_int
            ...
            % Add the facemorph param column..
            tdf_out(countr,17) = num2cell(MorphParMat(ii,zz));
    ...
    % add the header names to the output file columns
    headers = {"ImgFile_Path","FaceMorph_Con","Sal_Con","Config_Con","Image","Randomization_Col","Correct_RespKey", "T_Resp","T_Trial","T_Trial_wITI","Subj_RespKey","Correctness","tStart_Raw","tEnd_Raw","tITIstart_Raw","tITIend_Raw","MorphPar"};
    ...
    elseif db_mode == 1 && dbmode_prtThrsh == 1
        ...
        speechend = sprintf('This block has been completed. For threshold value: See command window. Please hit ENTER to continue.');
