...
%     end
    ...
    for trials = 1:numtrials
        ...
        elseif link=='F'
            param1(trials)=quant(10^(QuestQuantile(q1)),0.1);  % get next level of parameter to test
            %param1(trials)=QuestQuantile(q1);
        ...
        % if this is the first pass, start at the midpoint between max/min
        if trials==1
            param1(trials)=tGuess1;
        end
...
%                     noiseImgIn = noise_array{noise_arrayIdx(1,1),1}; %get the noise image for the index on top..
        ...
        %q1 = QuestUpdate(q1,param1(trials),response(trials,1));
        ...
        if n_int>1
            ...
            elseif link=='F'
                param2(trials)=quant(10^(QuestQuantile(q2)),0.1);  % get next level of parameter to test
                %param2(trials)=QuestQuantile(q2);
            end
...
%                     noiseImgIn = noise_array{noise_arrayIdx(1,1),1}; %get the noise image for the index on top..
            ...
            %q2 = QuestUpdate(q2,param2(trials),response(trials,2));
        end
        %% Staircase 3
        iQuestNum=3;
        if n_int>2
            ...
            elseif link=='F'
                param3(trials)=quant(10^(QuestQuantile(q3)),0.1);  % get next level of parameter to test
                %param3(trials)=QuestQuantile(q3);
            end
...
%                     noiseImgIn = noise_array{noise_arrayIdx(1,1),1}; %get the noise image for the index on top..
            ...
            %q3 = QuestUpdate(q3,param3(trials),response(trials,3));
        end
        %% Staircase 4
        if n_int>3
            if link=='L'
            ...
            elseif link=='F'
                param4(trials)=quant(10^(QuestQuantile(q4)),0.1);  % get next level of parameter to test
                %param4(trials)=QuestQuantile(q4);
            end
...
%                     noiseImgIn = noise_array{noise_arrayIdx(1,1),1}; %get the noise image for the index on top..
            ...
            %q4 = QuestUpdate(q4,param4(trials),response(trials,4));
        end
        %% Staircase 5
        if n_int>4
            if link=='L'
            ...
            elseif link=='F'
                param5(trials)=quant(10^(QuestQuantile(q5)),0.1);  % get next level of parameter to test
                %param5(trials)=QuestQuantile(q5);
            end
...
%                     noiseImgIn = noise_array{noise_arrayIdx(1,1),1}; %get the noise image for the index on top..
                    ...
                    if swtch2 == 0
                        ...
                        if nresp>1
                            respns=respns{randi(nresp)};
                            resp_mat(trials,5) = respns; % in edge case of simultaneous multiple button press.. make a random selection
                        else
            ...
            %q5 = QuestUpdate(q5,param5(trials),response(trials,5));
        end
    end
