function [trialtm,ts_out,qs_out,sd_out,nr_ideals, params_out, tdf_out] = FACEquest5_int_v2(room,kboard,numtrials,link,noise,window,image_dir,db_mode,db_mode_screen,nr_params, imtag, pthrds, n_int, qst_startParams,t_prob,maxmin,maxmin_seg,main_dir,olCon,parTagz,obj1Par,obj2Par,qstParStr)
...
%     newres = res1(res4(rmax));
    ...
    switch link
        case 'F' %
            ...
            par_inxsC1 = horzcat(par_inxsC1,cell2mat(img_arrayC1(1:end,6))); % add on the difference column 

            par_inxsC2 = cell2mat(img_arrayC2(1:end,3:4));
            par_inxsC2 = horzcat(par_inxsC2,cell2mat(img_arrayC2(1:end,6))); % add on the difference column 
            %#############################################################
...
% AND NOW THE PRACTICE TRIALS...
        ...
        switch link
            case 'F'
                ...
                if tcon==trial_con(2)
                    ...
                    ObjParMat(trials,1:end,1)=par_inxsC2(idx,1:2);
                    %!!??
                ...
                if tcon==trial_con(1)
                    ...
                    ImParMat(trials,1) = closest_param;
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,1)=par_inxsC1(idx,1:2);
                    %!!??
        ...
        if n_int>1
            ...
            switch link
                ...
                if tcon==trial_con(2)
                    ...
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,2)=par_inxsC2(idx,1:2);
                    %!!??
                    image_in = img_arrayC2{idx,1};
                end
                ...
                if tcon==trial_con(1)
                    ...
                    ImParMat(trials,2) = closest_param;
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,2)=par_inxsC1(idx,1:2);
                    %!!??
                    image_in = img_arrayC1{idx,1};
                end
        ...
        if n_int>2
            ...
            switch link
                ...
                if tcon==trial_con(2)
                    ...
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,3)=par_inxsC2(idx,1:2);
                    %!!??
                    image_in = img_arrayC2{idx,1};
                end
                ...
                if tcon==trial_con(1)
                    ...
                    ImParMat(trials,3) = closest_param;
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,3)=par_inxsC1(idx,1:2);
                    %!!??
                    image_in = img_arrayC1{idx,1};
                end
            ...
            switch link
                case 'F'
                 ...
                 if tcon==trial_con(2)
                    ...
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,4)=par_inxsC2(idx,1:2);
                    %!!??
                    image_in = img_arrayC2{idx,1};
                end
                ...
                if tcon==trial_con(1)
                    ...
                    ImParMat(trials,4) = closest_param;
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,4)=par_inxsC1(idx,1:2);
                    %!!??
                    image_in = img_arrayC1{idx,1};
                end
        ...
        if n_int>4
            ...
            switch link
                ...
                if tcon==trial_con(2)
                    ...
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,5)=par_inxsC2(idx,1:2);
                    %!!??
                    image_in = img_arrayC2{idx,1};
                end
                ...
                if tcon==trial_con(1)
                    ...
                    ImParMat(trials,5) = closest_param;
                    %Get the object parameters at index idx too..
                    ObjParMat(trials,1:end,5)=par_inxsC1(idx,1:2);
                    %!!??
                    image_in = img_arrayC1{idx,1};
                end
    ...
    tdf_out = cell(numtrials*n_int,(17+nParz));
    ...
    for ii = 1:numtrials
        for zz = 1:n_int
            ...
            tdf_out(countr,17) = num2cell(ImParMat(ii,zz)); %Difference Parameter
            % Raw Object Parameters
            tdf_out(countr,18:end)=num2cell(ObjParMat(ii,1:end,zz));
            countr = countr+1;
        end 
    end
    clear countr
    ...
    headers = {"ImgFile_Path","QUEST_Con","emptyCol","emptyCol","Image","Randomization_Col","Correct_RespKey", "T_Resp","T_Trial","T_Trial_wITI","Subj_RespKey","Correctness","tStart_Raw","tEnd_Raw","tITIstart_Raw","tITIend_Raw",obj1Par,obj2Par,qstParStr};
