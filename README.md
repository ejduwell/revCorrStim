...
## Data Output:
  ...
  - Later runs collected using that ID will be assumed to belong to the same subject and are saved under subdirectories within. 
  ...
  - XXX (name matching subject-id)
      - Within the main subject folder, there are seperate subdirectories for runs collected with white (white) and kernel (krnlNz) noise
          - Within each of those are separate subdirectories for runs collected using the luminance (lum) , texture (tex), and common region (cr) stimulus versions
              - Within each of these are subdirectories for sessions run with occluded base images (occ), non-occluded base images (nocc), or both (both)
  ...
  ### In the XXX_TIMESTAMP_cmpltd directory for each completed block:
  - The subject's data are saved in a .mat file named XXX.mat (where 'XXX' is the subject's unique ID)
  ...
  - Columns 19:end will vary based on the stimulus version you run, however in the case above they represent:
      ...
      - 22 "CR" : indicates whether the texture aspect was on or off
      - 23 "T" : indicates whether the common region aspect was on or off

  #### Autogenerated plots and figs
