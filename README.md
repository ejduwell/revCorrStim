...
### Data Output:
  ...
  Separate data output subdirectories matching the subject's ID are automatically generated the first time the subject ID is used.

  Later runs collected using that ID will be assumed to belong to that subject/ are saved under subdirectories within. SO MAKE SURE YOUR IDS ARE UNIQUE FOR EACH SUBJECT/ARE NOT RE-USED!

  Within that folder seperate subdirectories for runs with white and kernel noise are also autogenerated

  Within each of those separate subdirectories for the luminance (lum) , texture (tex), and common region (cr) versions' data are also autogenerated

  Within each of these subdirectories for sessions run with occluded base images (occ), non-occluded base images (nocc), or both (both) are also autogenerated
  ...
  The subjects data are saved in a .mat file named XXX.mat (where 'XXX' is the subject's unique ID)
